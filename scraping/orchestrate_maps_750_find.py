...
CENTROIDS_CSV = Path("jakarta_centroids_750.csv")  # при желании подставьте другое имя
...
RADIUS = 600
LANG = "en"
USE_FAST_MODE = True          # fast mode включён
...
# 270 KiB
MIN_BYTES_THRESHOLD = 270 * 1024

# Один проход (без ретраев)
DEPTH = 15
WARMUP_SECONDS = 30          # прогрев
STALL_WINDOW_SECONDS = 30    # окно «застоя»
EXIT_ON_INACTIVITY = "2m"    # внутренняя страховка скрэпера
...
# FOLLOW_LOGS=1 — включить поток docker logs -f (по умолчанию выкл).
...
# df = df.iloc[195:]  # при необходимости оставьте срез; по умолчанию весь файл
...
def follow_logs(name, idx):
    """Опциональный поток для логов: docker logs -f NAME (по умолчанию отключён)."""
    if not FOLLOW_LOGS:
        return
    try:
        p = subprocess.Popen(
...
def run_once(idx, lat, lon, results_file):
    ...
    _log(f"[{idx}] docker run: {' '.join(cmd)}")
    ...
    # Стартуем поток логов (если FOLLOW_LOGS=1)
